...
- `init`: create a SQLite manifest under the job directory.
...
- Regression test suite covering scan/excludes, copy, resume, timeout/failure continuation, symlink safety, temp collision, streamed manifest selection, and limit starvation.
...
2. Add a source/destination nesting guard so `--dest` cannot be inside `--source`.
